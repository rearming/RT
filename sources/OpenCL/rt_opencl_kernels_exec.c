		...
		const size_t *work_size)
{
	...
	rt_update_render_params(render_kernel, &rt->params, rt->render_options,
			rt->render_actions);
...
void		exec_depth_of_field_kernel(
		t_rt *rt,
		cl_kernel kernel,
		const size_t *work_size)
{
