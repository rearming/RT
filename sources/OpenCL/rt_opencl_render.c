...
void		rt_opencl_render(void *rt_ptr)
{
	...
	exec_render_kernel(rt, rt_get_render_kernel(rt->render_options), global_work_size);
	exec_depth_of_field_kernel(rt, g_opencl.kernels[KERNEL_DOF], global_work_size);
	exec_sepia_kernel(rt, g_opencl.kernels[KERNEL_SEPIA], global_work_size);
	exec_cartoon_kernel(rt, g_opencl.kernels[KERNEL_CARTOON], global_work_size);
