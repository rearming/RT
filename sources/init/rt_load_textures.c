...
void		rt_textures_init(void)
{
	...
	{
		if ((tmp_filename =
			found_file_in_folder(g_textures.folders_names, g_textures.textures_names->name))
			== NULL)
...
void		rt_skybox_init(void)
{
	...
	j = -1;
	if ((tmp_filename =
		found_file_in_folder(g_textures.folders_names, g_textures.skybox_info->skybox_name))
		== NULL)
		return (rt_raise_error(ERR_INVALID_TEXTURE));
