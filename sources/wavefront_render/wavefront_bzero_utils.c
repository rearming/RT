...
void		bzero_buffer(enum e_cl_mem_types mem_index)
{
	...
	err |= clEnqueueWriteBuffer(g_opencl.queue, g_opencl.wf_shared_buffers[mem_index].mem,
...
void		bzero_float3_temp_img(cl_float3 *zero_arr)
{
	...
	err |= clEnqueueWriteBuffer(g_opencl.queue, g_opencl.wf_shared_buffers[RT_CL_MEM_TEMP_FLOAT3_IMG_DATA].mem,
