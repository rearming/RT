		...
		t_rayhit *best_hit)
{
	...
	b = 2.0f * (dot(ray->dir, origin_center) - dot_ray_axis_paraboloid * (dot_origin_center_axis_paraboloid + 2.0f * paraboloid->distance));
	c = dot(origin_center, origin_center) - dot_origin_center_axis_paraboloid * (dot_origin_center_axis_paraboloid + 4.0f * paraboloid->distance);
	...
	if (root < 0 || (root > root2 && root2 > 0.0f)) /// пересечение перед камерой,
		...
		t_rayhit *best_hit)
{
	...
	const float		distance_2 = ellipsoid->distance * 2.0f;
	...
	a = 4.0f * radius_pow_2 * dot(ray->dir, ray->dir) - a_1 * a_1;
	b = 2.0f * (4.0f * radius_pow_2 * dot(ray->dir, origin_center) - a_1 * a_2);
	c = 4.0f * radius_pow_2 * dot(origin_center, origin_center) - a_2 * a_2;
	...
	if (root < best_hit->distance && root > RAY_MIN_EPSILON && root > 0.0f)
	{
		...
		best_hit->normal = fast_normalize(center_mid_pos + ellipsoid->axis
				* (1.0f - b * b / (4.0f * a * a)) * dot(center_mid_pos, ellipsoid->axis));
		return true;
	}
	return false;
}