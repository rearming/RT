...
static inline void		add_key_event(t_events *events, SDL_Scancode scancode)
{
...
void		handle_event(SDL_Event *event, t_rt *rt)
{
	...
	else if (event->type == SDL_KEYDOWN)
	{
		...
		add_key_event(&rt->events, event->key.keysym.scancode);
	}
	...
	if (any_key_pressed(&rt->events) || event->type == SDL_MOUSEMOTION)
	{
