...
static	t_object	*pathtrace_objects(int *out_obj_nbr)
{
	...
	*out_obj_nbr = objects_nbr;
	...
	objects[0] = (t_object){.type = SPHERE,
			(t_material){.albedo = get_float3_color(COL_DARK_RED), .specular = {{0.7f, 0.7f, 0.7f}}, .smoothness = 100},
	...
	objects[2] = (t_object){.type = SPHERE, /// golden sphere
			(t_material){.albedo = get_float3_color(COL_GOLD), .specular = {{1.0f, 1.0f, 1.0f}}, .smoothness = 2000.f},
			.center = (cl_float3){{5, 1, 3}},
			.radius = 1};
	objects[3] = (t_object){.type = SPHERE,
			(t_material){.albedo = get_float3_color(COL_LIGHT_PURPLE), .specular = {{1.0f, 1.0f, 1.0f}}, .smoothness = 20},
	...
	objects[5] = (t_object){.type = SPHERE,
			...
			.radius = 0.8};
	objects[6] = (t_object){.type = SPHERE,
			(t_material){.albedo = get_float3_color(COL_WHITE), .specular = {{1.0f, 1.0f, 1.0f}}, .smoothness = MAX_SMOOTHNESS},
	...
	objects[10] = (t_object){.type = SPHERE,
			(t_material){.albedo = get_float3_color(COL_WHITE), .specular = {{1.f, 1.f, 1.f}},
...
//	lights[1] = (t_light){.pos = {{0, 10, -5}}, .color = get_float3_color(COL_WHITE), .type = POINT, .intensity = 0.5f};
	lights[1] = (t_light){.pos = {{0, 0, -5}}, .color = get_float3_color(COL_WHITE), .type = POINT, .intensity = 0.5f};
