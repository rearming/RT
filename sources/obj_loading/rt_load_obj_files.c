...
static inline char		*read_obj_file(
...
static inline bool		get_raw_obj(
		const char *path_to_obj,
		t_raw_obj *out_raw_obj)
{
	t_raw_obj	raw_obj;
	int			err;
	size_t		size;
	...
	if (!obj_data)
		return (false);
	err = tinyobj_parse_obj(&raw_obj.attrib, &raw_obj.shapes,
			(size_t*)&raw_obj.num_shapes, &raw_obj.materials,
			(size_t*)&raw_obj.num_materials, obj_data, size,
			RT_OBJLOADER_NOFLAGS);
