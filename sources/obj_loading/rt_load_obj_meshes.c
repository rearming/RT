...
t_polygon		rt_get_polygon(t_raw_obj *raw_obj, int polygon_i)
{
	...
	if (raw_obj->attrib.face_num_verts[polygon_i]
	!= RT_DEFAULT_POLYGON_VERTICES)
		...
		int polygon_i)
{
	...
	while (i < num_shapes)
	{
		if (polygon_i >= (int)shapes[i].face_offset
		&& polygon_i < (int)shapes[i].length + (int)shapes[i].face_offset)
			return (i);
		i++;
	}
	return (0);
...
t_polygon		*rt_get_all_polygons(t_raw_obj *raw_obj)
{
	...
	if (raw_obj->attrib.num_faces < 1)
		return (NULL);
	polygons = rt_safe_malloc(
			raw_obj->attrib.num_faces / 3 * sizeof(t_polygon));
	i = 0;
	while (i < raw_obj->attrib.num_faces / 3)
	{
		polygons[i] = rt_get_polygon(raw_obj, i);
		polygons[i].mesh_index = get_mesh_index(
				raw_obj->shapes, raw_obj->num_shapes, i);
		i++;
	}
	return (polygons);
}
		...
		if (raw_obj->attrib.material_ids[i] == NOT_SET)
			...
			mesh_info[i].material =
				rt_convert_obj_material(rt_get_obj_material(
						&raw_obj->materials[raw_obj->attrib.material_ids
						[material_ids_id]]));
...
void			rt_get_meshes(t_raw_obj *raw_obj, t_meshes *out_meshes)
{
	...
	out_meshes->vertices = rt_get_vertices(
			raw_obj->attrib.vertices, raw_obj->attrib.num_vertices);
	out_meshes->v_normals = rt_get_vertices(
			raw_obj->attrib.normals, raw_obj->attrib.num_normals);
	out_meshes->v_textures = rt_get_texture_vertices(
			raw_obj->attrib.texcoords, raw_obj->attrib.num_texcoords);
